(()=>{
  const moveInstall=()=>{
    ...
    if(!main)return;
    const install=[...document.querySelectorAll('button,a,[data-install-app]')].find(el=>/install pijush/i.test(el.textContent||'')||el.matches('[data-install-app]'));
    if(!install)return;
    ...
    if(slot && install.parentElement!==slot){
      slot.appendChild(install);
      install.classList.add('install-in-section');
      install.removeAttribute('style');
    }
  };
  const style=document.createElement('style');
  style.textContent=`.install-section{padding:95px 0}.install-card{display:flex;align-items:center;justify-content:space-between;gap:20px;padding:24px;border:1px solid var(--line);border-radius:24px;background:var(--surface)}.install-card strong{display:block;font:600 22px 'Space Grotesk'}.install-card span{display:block;color:var(--muted);font-size:12px;margin-top:5px}.install-slot{display:flex;justify-content:flex-end}.install-in-section{position:static!important;left:auto!important;right:auto!important;top:auto!important;bottom:auto!important;z-index:auto!important;margin:0!important;display:inline-flex!important;max-width:100%!important;white-space:nowrap!important}.install-card .install-in-section{min-height:48px;padding:13px 20px;border-radius:999px}@media(max-width:560px){.install-section{padding:75px 0}.install-card{display:grid;gap:18px}.install-slot{justify-content:stretch}.install-card .install-in-section{width:100%;justify-content:center;white-space:normal!important}.install-card span{line-height:1.5}}`;
  document.head.appendChild(style);
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',moveInstall);else moveInstall();
  const observer=new MutationObserver(()=>moveInstall());
